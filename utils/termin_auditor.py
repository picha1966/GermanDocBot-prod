...
# Cooldown: suppress re-alerting the same set of problem cities for this long
ALERT_COOLDOWN_MIN: int = 60

# ---------------------------------------------------------------------------
# In-memory cooldown state (resets on restart — acceptable for 24h cycle)
# ---------------------------------------------------------------------------
_last_alert_at: Optional[datetime]  = None
_last_problem_key: Optional[str]    = None   # sorted join of problem city names
...
# Cooldown check
# ---------------------------------------------------------------------------
def _should_send_alert(problem_cities: List[str]) -> bool:
    """
    Return False if the exact same set of problem cities was already alerted
    within ALERT_COOLDOWN_MIN minutes (prevents repeat spam during long outages).
    """
    global _last_alert_at, _last_problem_key

    current_key = ",".join(sorted(problem_cities))
    now         = datetime.now()

    if (
        _last_problem_key == current_key
        ...
        logger.info(
            "AUDITOR_COOLDOWN | same problems as %d min ago — suppressing alert | cities=%s",
            minutes_ago, problem_cities,
        )
        return False
...
async def run_full_audit(bot) -> None:
    ...
    if not problem_cities:
        ...
        if _should_send_alert(problem_cities):
...
async def termin_audit_loop(bot) -> None:
    ...
    logger.info(
        "AUDITOR_LOOP_START | interval=%dh | cities=%s",
        AUDIT_INTERVAL_HOURS, list(_AUDIT_TARGETS),
    )
    while True:
        try:
