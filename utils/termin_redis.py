...
def _ensure_client():
    ...
    _init_done is only set to True for permanent failures (no URL, missing
    package) — a transient connection error leaves it False so the next
    ...
    if not url:
        _init_done = True  # permanent: no URL configured
    ...
    try:
        ...
        _client.ping()
        _init_done = True  # permanent: connection confirmed
        logger.info("TERMIN_REDIS | Connected to Redis at %s", url.split("@")[-1])
    except ImportError:
        _init_done = True  # permanent: package will not appear without a deploy
        logger.warning("TERMIN_REDIS | redis package not installed — in-memory fallback")
        _client = None
    except Exception as exc:
        # Transient failure — leave _init_done=False so the next call retries.
        logger.warning("TERMIN_REDIS | Connection failed (%s) — will retry on next access", exc)
        _client = None
    return _client
...
def rget(key: str) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("TERMIN_REDIS | rget(%s) error: %s", key, _e)
        return None
...
def rset(key: str, value: str, ttl: int = 0) -> bool:
    ...
    try:
        ...
        logger.debug("TERMIN_REDIS | rset(%s) error: %s", key, _e)
        return False
...
def rdel(key: str) -> bool:
    ...
    try:
        ...
        logger.debug("TERMIN_REDIS | rdel(%s) error: %s", key, _e)
        return False
