...
def build_tevis_service_url(base_url: str, md: int) -> str:
    """Return the public booking URL for a TeVIS service.

    The portal root is the safest stateless link — select2?md=N is an internal
    wizard step that requires an active session and must NOT be shown to users.
    """
...
async def _navigate_to_location_page(page, base_url: str, md: int) -> bool:
    ...
    try:
        ...
        if not cnc_id:
            logger.warning("No cnc_id found on step 2 — cannot build location URL")
            return False
        ...
        if "Schritt 3" in html3 or "select_location" in html3:
            ...
            return True
        logger.warning("Step 3 page content unexpected after navigation")
        return False
