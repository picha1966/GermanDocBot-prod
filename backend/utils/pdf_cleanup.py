...
def cleanup_old_pdfs(max_age_hours: int = 2, skip_paths: set = None) -> int:
    """
    Delete PDF files in the generated_pdfs/ directory that are older than
    max_age_hours. Non-PDF files are never touched.

    skip_paths: set of absolute path strings to never delete — used to protect
    PDFs for orders where email has not been sent yet (email_sent=0).
    ...
    _skip = {str(Path(p).resolve()) for p in (skip_paths or set()) if p}
    ...
    for pdf_file in _OUTPUT_DIR.iterdir():
        ...
        if pdf_file.suffix.lower() != ".pdf":
            continue
        if str(pdf_file.resolve()) in _skip:
            logger.debug("pdf_cleanup: skipping %s — email not yet sent", pdf_file.name)
            continue
        try:
