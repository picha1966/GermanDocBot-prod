...
class Database:
    ...
    def _create_tables(self) -> None:
        ...
        try:
            cursor.execute("ALTER TABLE orders ADD COLUMN email_sending INTEGER DEFAULT 0")
            logger.debug("✅ Added email_sending column to orders table")
        except sqlite3.OperationalError:
            pass  # Column already exists
        ...
        try:
            cursor.execute(
                "ALTER TABLE orders ADD COLUMN retention_followups_scheduled INTEGER DEFAULT 0"
            )
            logger.debug("✅ Added retention_followups_scheduled column to orders table")
        except sqlite3.OperationalError:
            pass  # Column already exists

        try:
            cursor.execute(
                "ALTER TABLE orders ADD COLUMN email_capture_offered INTEGER DEFAULT 0"
            )
            logger.debug("✅ Added email_capture_offered column to orders table")
        except sqlite3.OperationalError:
            pass  # Column already exists
    ...
    def try_claim_retention_schedule(self, order_id: int) -> bool:
        """
        Idempotent: first caller after PDF delivery schedules post-payment retention tasks.
        Webhook retries must not enqueue duplicate asyncio retention chains.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                """
                UPDATE orders
                SET retention_followups_scheduled = 1
                WHERE id = ?
                  AND (retention_followups_scheduled IS NULL OR retention_followups_scheduled = 0)
                """,
                (int(order_id),),
            )
            self.conn.commit()
            ...
            logger.error("try_claim_retention_schedule failed order_id=%s err=%s", order_id, e)
            return False
    ...
    def claim_delivery(self, order_id: int) -> bool:
        ...
        try:
            ...
            # Re-raise so the caller treats this as a hard failure (not as "already claimed").
            # deliver_document_after_payment will catch it, mark FAILED, and schedule retry.
            raise

    def claim_paid(self, order_id: int, stripe_session_id: Optional[str] = None) -> bool:
        """Atomically transition order PENDING → PAID.

        Returns True only if this call won the race (rowcount > 0).
        Returns False if the row moved to another status between our idempotency
        check and this write (TOCTOU guard — closes the webhook/deeplink race).
        """
        try:
            cursor = self.conn.cursor()
            if stripe_session_id:
                cursor.execute(
                    "UPDATE orders SET status = ?, stripe_session_id = ?, "
                    "paid_at = CURRENT_TIMESTAMP "
                    "WHERE id = ? AND status = ?",
                    (OrderStatus.PAID.value, stripe_session_id, order_id, OrderStatus.PENDING.value),
                )
            else:
                cursor.execute(
                    "UPDATE orders SET status = ?, paid_at = CURRENT_TIMESTAMP "
                    "WHERE id = ? AND status = ?",
                    (OrderStatus.PAID.value, order_id, OrderStatus.PENDING.value),
                )
            self.conn.commit()
            won = cursor.rowcount > 0
            if won:
                logger.info("claim_paid WON: order_id=%s PENDING→PAID", order_id)
            else:
                logger.info("claim_paid LOST: order_id=%s — status moved since idempotency check", order_id)
            return won
        except Exception as exc:
            logger.error("claim_paid failed: order_id=%s err=%s", order_id, exc)
            return False
    ...
    def claim_email_send(self, order_id: int) -> bool:
        ...
        Sets email_sending=1 (in-flight marker). email_sent remains 0 until
        confirm_email_sent() is called after a successful send.

        Two-flag design:
          email_sending=1, email_sent=0 → send in progress (or crashed mid-send)
          email_sending=0, email_sent=1 → send confirmed successful
          email_sending=0, email_sent=0 → not yet claimed (safe to claim)

        Startup recovery resets email_sending=1 rows and retries them, closing
        the crash window between claim and send without risk of duplicates.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE orders SET email_sending = 1"
                " WHERE id = ?"
            ...
            won = cursor.rowcount > 0
            if won:
                logger.info("claim_email_send WON: order_id=%s email_sending=1", order_id)
            else:
                logger.info("claim_email_send LOST: order_id=%s — already sent or in-flight", order_id)
            return won
        except Exception as exc:
            logger.error("claim_email_send failed: order_id=%s err=%s", order_id, exc)
            return False

    def confirm_email_sent(self, order_id: int) -> None:
        """Mark email as successfully delivered: email_sent=1, email_sending=0.

        Must be called after send_pdf_by_email() returns True.
        Separating confirmation from claim prevents permanent loss when the
        process crashes after claim but before the actual send completes.
        Best-effort — never raises.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE orders SET email_sent = 1, email_sending = 0"
                " WHERE id = ? AND email_sending = 1",
                (order_id,),
            )
            self.conn.commit()
            if cursor.rowcount > 0:
                logger.info("confirm_email_sent: order_id=%s email_sent=1 email_sending=0", order_id)
            else:
                logger.warning(
                    "confirm_email_sent NOOP: order_id=%s — email_sending was not 1 (no claim or already confirmed)",
                    order_id,
                )
        except Exception as exc:
            logger.error("confirm_email_sent failed: order_id=%s err=%s", order_id, exc)

    def release_email_claim(self, order_id: int) -> None:
        """Reset email_sending=0 so the next attempt can retry sending.

        Called when claim_email_send() won but the send failed (provider error,
        timeout, PDF missing). email_sent is NOT touched — it was never set to 1.
        Best-effort — never raises.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                ...
                " WHERE id = ? AND (email_sent IS NULL OR email_sent = 0)",
                (order_id,),
            )
            self.conn.commit()
            logger.info("release_email_claim: order_id=%s email_sending reset 0", order_id)
        except Exception as exc:
            logger.error("release_email_claim failed: order_id=%s err=%s", order_id, exc)

    def claim_email_capture_offer(self, order_id: int) -> bool:
        """Atomic idempotency guard for send_email_capture_offer.

        Sets email_capture_offered=1 only when it is currently 0 (or NULL).
        Returns True if this call won the claim (safe to send the offer).
        Returns False if the offer was already sent (skip duplicate).
        Best-effort — returns False on any DB error.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                "UPDATE orders SET email_capture_offered = 1"
                " WHERE id = ? AND COALESCE(email_capture_offered, 0) = 0",
                (order_id,),
            )
            self.conn.commit()
            won = cursor.rowcount == 1
            if won:
                logger.info("claim_email_capture_offer WON: order_id=%s", order_id)
            else:
                logger.info("claim_email_capture_offer LOST: order_id=%s — already offered", order_id)
            return won
        except Exception as exc:
            logger.error("claim_email_capture_offer failed: order_id=%s err=%s", order_id, exc)
            return False

    def save_customer_email(self, order_id: int, email: str, *, overwrite: bool = False) -> None:
        """Persist customer email on the order row (best-effort, never raises).

        When overwrite is False (default), only fills an empty customer_email.
        When True (e.g. Stripe checkout), always set to the latest confirmed address.
        """
        try:
            cursor = self.conn.cursor()
            if overwrite:
