...
class StripePaymentHandler:
    ...
    async def create_checkout_session(
        ...
        try:
            ...
            # Webhook can read session.metadata.email even when Checkout collects email later.
            if customer_email and str(customer_email).strip():
                _cem = str(customer_email).strip()
                if '@' in _cem:
                    metadata['email'] = _cem
            ...
            # Pre-fill when we have it; if omitted, Checkout asks for email (required for receipt).
            # Do not rely on this alone — customer_creation=always + Customer object improves wallet flows.
            if customer_email and str(customer_email).strip():
                _ce = str(customer_email).strip()
                if '@' in _ce:
                    session_params['customer_email'] = _ce
            ...
            # Log API key mode before creating session (critical for test/live mismatch diagnosis)
            _api_key = stripe.api_key or ""
            _key_mode = "LIVE" if _api_key.startswith("sk_live") else ("TEST" if _api_key.startswith("sk_test") else "UNKNOWN")
            logger.info(
                "STRIPE_SESSION_CREATE | order_id=%s user_id=%s doc_type=%s price=%.2f mode=%s key_mode=%s",
                order_id, user_id, doc_type, float(final_price), session_params["mode"], _key_mode,
            )
            print(f"STRIPE_KEY_MODE={_key_mode} order_id={order_id}", flush=True)
            ...
            # Log session status immediately after creation (should be 'open' / 'unpaid')
            _sess_status = getattr(session, "status", None)
            _sess_payment_status = getattr(session, "payment_status", None)
            logger.info(
                "STRIPE_SESSION_CREATED | session_id=%s status=%s payment_status=%s url=%s",
                session.id, _sess_status, _sess_payment_status, session.url,
            )
            print(f"STRIPE_SESSION_CREATED session_id={session.id} status={_sess_status} payment_status={_sess_payment_status}", flush=True)
