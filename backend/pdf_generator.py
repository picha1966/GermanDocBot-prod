...
PREVIEW_FOOTER_ONE_LINE = {
    ...
    "uk": "Це превʼю. Неофіційний документ.",
...
def _has_meaningful_user_data_for_preview(user_data: Dict[str, Any]) -> bool:
    ...
    if not has_data:
        logger.warning(
            "PREVIEW_NO_MEANINGFUL_DATA | fields_total=%d doc skipped", len(user_data)
        )
...
# Delivery watermark for PAID PDFs — always German, no per-language variants.
# Reason: insert_textbox uses the built-in "helv" (Helvetica) font which covers
# Latin-1 only.  The previous localized strings (Cyrillic for uk/ua, Arabic for ar)
# rendered as "?????" because helv cannot encode those code-points.
# German is Latin-only and renders correctly without any embedded font.
DELIVERY_WATERMARK_TEXT = "NICHT OFFIZIELLES DOKUMENT - NUR ZUR VORBEREITUNG"


def _apply_delivery_watermark(pdf_path: str, user_lang: str) -> None:
    """
    Apply small red header text to final PDFs.

    Always uses the fixed German string DELIVERY_WATERMARK_TEXT regardless of
    user_lang — Latin-only text renders correctly with the built-in PDF font
    (Helvetica/helv).  Localized Cyrillic/Arabic strings caused "?????" output
    because no Unicode font was embedded.
    """
    if not PYMUPDF_AVAILABLE:
        return
    _wm_text = DELIVERY_WATERMARK_TEXT
    try:
...
def _apply_red_disclaimer(pdf_path: str, doc_type: str = "", mode: str = "") -> None:
    """
    Mandatory red disclaimer for ALL generated PDFs — preview and final.

    Applies DELIVERY_WATERMARK_TEXT ("NICHT OFFIZIELLES DOKUMENT - NUR ZUR VORBEREITUNG")
    in red on every page using the built-in Helvetica font (ASCII-safe, no encoding issues).

    This is the single centralized entry point for the disclaimer requirement.
    Called on every path that does NOT already invoke _apply_delivery_watermark.
    Raises on failure so callers can decide whether to abort delivery.
    """
    try:
        _apply_delivery_watermark(pdf_path, "de")
    except Exception as _e:
        logger.error(
            "PDF_DISCLAIMER_FAILED | doc_type=%s | mode=%s | path=%s | error=%s",
            doc_type, mode, pdf_path, _e,
        )
        raise
    logger.info(
        "PDF_DISCLAIMER_APPLIED | doc_type=%s | mode=%s | path=%s",
        doc_type, mode, pdf_path,
    )
...
def _fill_template_pdf_overlay(
    ...
    try:
        for field_key, cfg in overlay_map.items():
            ...
            logger.debug(
                "_fill_template_pdf_overlay: wrote field=%r page=%d at (%.1f, %.1f)",
                field_key,
                page_idx,
                x,
                y,
...
def _fill_template_pdf_acroform(
    ...
    try:
        ...
        if doc_lower in ("buergergeld", "jobcenter"):
            ...
            logger.warning(
                "[FINAL GUARD] doc=%s sv_radio=%r has_sv=%s iban=%s chbxKonto=%r",
                doc_type,
                _sv_radio,
                bool(_sv_text),
                bool(_iban),
            ...
            if _sv_text and _sv_radio == "1":
                logger.error(
                    "[FINAL GUARD FAIL] doc=%s has_iban=%s chbxKonto=%r "
                    "sv_radio=%r has_sv=%s — SV number present but Nein selected",
                    doc_type,
                    bool(_iban),
                    _chbx_konto,
                    _sv_radio,
            ...
            if not _sv_text and _sv_radio == "0":
                logger.error(
                    "[FINAL GUARD FAIL] doc=%s has_iban=%s chbxKonto=%r "
                    "sv_radio=%r has_sv=%s — no SV number but Ja selected",
                    doc_type,
                    bool(_iban),
                    _chbx_konto,
                    _sv_radio,
            ...
            if _iban and _chbx_konto == "YES_CHECKED":
                logger.error(
                    "[FINAL GUARD FAIL] doc=%s has_iban=%s chbxKonto=%r "
                    "sv_radio=%r has_sv=%s — IBAN present but 'keine Bankverbindung' is checked",
                    doc_type,
                    bool(_iban),
                    _chbx_konto,
                    _sv_radio,
            ...
            if (
                ...
                logger.error(
                    "[FINAL GUARD FAIL] doc=%s has_iban=%s chbxKonto=%r "
                    "sv_radio=%r has_sv=%s — no IBAN but 'keine Bankverbindung' not checked",
                    doc_type,
                    bool(_iban),
                    _chbx_konto,
                    _sv_radio,
        ...
        for page_num in range(len(pdf)):
            ...
            for w in page.widgets():
                ...
                try:
                    ...
                    filled += 1
                    if _is_critical:
                        logger.warning("[PDF WRITE] %s (value omitted)", acroform_name)
                    else:
                        logger.debug(
...
def create_preview(
    ...
    if (doc_type or "").strip().lower() == "kindergeld":
        try:
            ...
            if _kg_prev_result:
                ...
                _apply_red_disclaimer(_kg_prev_result, doc_type, "preview")
                logger.info(
    ...
    try:
        ...
        if fb_result:
            # Add grey footer disclaimer + mandatory red header to preview PDFs.
            ...
            _apply_red_disclaimer(fb_result, doc_type, "preview")
            return fb_result
    ...
    if _emergency_result:
        _apply_red_disclaimer(_emergency_result, doc_type, "preview_emergency")
...
def create_final_pdf(
    ...
    if _doc_key == "anmeldung":
        try:
            ...
            if _tmpl_result:
                _apply_red_disclaimer(_tmpl_result, doc_type, "final")
                return _tmpl_result
            logger.warning(
    ...
    if _doc_key == "kindergeld":
        try:
            ...
            _apply_red_disclaimer(str(_kg_output), doc_type, "final")
    ...
    _ud_keys = {k.lower() for k in user_data}
    logger.info(
        "PDF_DATA_SUMMARY | doc_type=%s user_id=%s fields=%s "
        "has_first_name=%s has_last_name=%s has_birth_date=%s "
        "has_iban=%s has_phone=%s has_email=%s",
        doc_type,
        user_id,
        len(user_data),
        bool(user_data.get("first_name")),
        bool(user_data.get("last_name")),
        bool(user_data.get("birth_date") or user_data.get("birthday")),
        any("iban" in k for k in _ud_keys),
        any(k in _ud_keys for k in ("phone", "telefon", "mobil")),
        any("email" in k or "mail" in k for k in _ud_keys),
    )
    ...
    if (doc_type or "").strip().lower() in ("buergergeld", "jobcenter"):
        ...
        if _sv_flag_e in ("nein", "no", "false", "0") and _sv_num_e:
            logger.warning(
                "[ENFORCE] sv_number cleared | had_sv_number=%s has_sv_number=%s doc=%s user=%s",
                bool(_sv_num_e),
                _sv_flag_e,
                doc_type,
                user_id,
        ...
        # 5. Diagnostic log (values masked — no PII in logs)
        logger.warning(
            "[ENFORCE DATA] doc=%s has_iban=%s kein_konto_derived=%s "
            "sv_flag=%s has_sv_num=%s has_sig_date=%s "
            "has_birth_place=%s has_birth_country=%s",
            doc_type,
            bool(_iban_e),
            not bool(_iban_e),
            user_data.get("has_sv_number", "?"),
            bool(user_data.get("sv_number")),
            bool(user_data.get("signature_date")),
            bool(user_data.get("birth_place")),
            bool(user_data.get("birth_country")),
        )
    ...
    if callable(_premium_validate):
        try:
            ...
            logger.warning(
                "⚠️ create_final_pdf: validation SKIPPED due to exception — "
                "PDF may contain missing fields. doc_type=%s err=%s",
                doc_type,
                _ve,
            )
    ...
    if _render_strategy in ("builder_only", "xfa_builder"):
        ...
        if template_path:
            ...
            if (doc_type or "").strip().lower() in ("buergergeld", "jobcenter"):
                ...
                logger.warning(
                    "[FINAL DATA BEFORE PDF] doc=%s has_iban=%s kein_konto_derived=%s "
                    "sv_flag=%s has_sv_num=%s has_sig_date=%s "
                    "has_birth_place=%s has_birth_country=%s",
                    doc_type,
                    bool(_iban_nd),
                    ...
                    bool(data_for_pdf.get("sv_number")),
                    bool(data_for_pdf.get("signature_date")),
                    bool(data_for_pdf.get("birth_place")),
                    bool(data_for_pdf.get("birth_country")),
                )

            # ── FINAL PIPELINE CHECK (always logged at WARNING level) ──────────
            logger.warning(
                "[FINAL PIPELINE CHECK] doc_type=%s has_template=%s template_path=%s "
                "has_iban=%s jc_kein_konto=%r has_sv_number=%s has_sv_number_value=%s",
                doc_type,
                has_template(doc_type) if callable(has_template) else "?",
                template_path,
                bool(data_for_pdf.get("iban")),
