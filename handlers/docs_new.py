...
from utils.funnel_log import funnel_city_from_mapping, log_funnel
from utils.retention import RETENTION_UPSELL_DOC_PREFIX
...
# Appended to preview captions (PNG snippet + PDF) — trust & clarity (HTML).
PREVIEW_TRUST_APPEND_HTML = {
    "uk": (
        "📄 <b>Це частина вашого згенерованого документа</b>\n"
        "✅ <b>Дані вже підставлені автоматично</b>\n"
        "💳 <b>Після оплати ви отримаєте повний PDF</b>\n"
        "⚠️ <b>Без правильного заповнення заявку можуть повернути</b>"
    ),
    "en": (
        "📄 <b>This is part of your generated document</b>\n"
        "✅ <b>Your details are filled in automatically</b>\n"
        "💳 <b>After payment you’ll receive the full PDF</b>\n"
        "⚠️ <b>If the form is wrong, your application may be rejected</b>"
    ),
    "de": (
        "📄 <b>Dies ist ein Teil Ihres generierten Dokuments</b>\n"
        "✅ <b>Die Daten wurden automatisch eingetragen</b>\n"
        "💳 <b>Nach der Zahlung erhalten Sie das vollständige PDF</b>\n"
        "⚠️ <b>Bei falschen Angaben kann der Antrag zurückgewiesen werden</b>"
    ),
    "pl": (
        "📄 <b>To jest fragment wygenerowanego dokumentu</b>\n"
        "✅ <b>Dane zostały uzupełnione automatycznie</b>\n"
        "💳 <b>Po płatności otrzymasz pełny PDF</b>\n"
        "⚠️ <b>Przy błędnym wypełnieniu wniosek może zostać odrzucony</b>"
    ),
    "tr": (
        "📄 <b>Bu, oluşturulan belgenizin bir bölümüdür</b>\n"
        "✅ <b>Veriler otomatik olarak dolduruldu</b>\n"
        "💳 <b>Ödeme sonrası tam PDF’yi alırsınız</b>\n"
        "⚠️ <b>Yanlış doldurulursa başvuru reddedilebilir</b>"
    ),
    "ar": (
        "📄 <b>هذا جزء من المستند الذي تم إنشاؤه</b>\n"
        "✅ <b>تم تعبئة البيانات تلقائيًا</b>\n"
        "💳 <b>بعد الدفع ستستلم ملف PDF كاملًا</b>\n"
        "⚠️ <b>قد يُرفض الطلب إذا كان التعبئة غير صحيحة</b>"
    ),
}


def _append_preview_trust_html(
    caption: str, lang: Optional[str], user_id: Optional[int] = None
) -> str:
    l = _norm_lang(lang, user_id)
    block = PREVIEW_TRUST_APPEND_HTML.get(l, PREVIEW_TRUST_APPEND_HTML["en"])
...
async def handle_webapp_data(message: types.Message):
    ...
    # Trace: log birth_place presence (value omitted) before normalization
    logger.info(
        "WEBAPP_BIRTH_PLACE_TRACE: user_id=%s doc_type=%s has_birth_place=%s has_child_birth_place=%s",
        user_id,
        doc_type,
        bool((answers or {}).get("birth_place")),
        bool((answers or {}).get("child_birth_place")),
    )
    ...
    log_funnel(
        ...
        doc_type=doc_type,
        lang=user_lang,
        city=funnel_city_from_mapping(answers),
    )
...
async def _send_draft_then_menu_via_bot(
    ...
    _draft_caption = _append_preview_trust_html(_draft_caption, user_lang, uid)
    _snippet_http_fallback = "🔍 Document preview — verify your name and date of birth."
    _snippet_caption_http = _append_preview_trust_html(
        {
            "uk": (
        ...
        }.get(user_lang, _snippet_http_fallback),
        user_lang,
        uid,
    )
...
async def cmd_testpdf(message: types.Message):
    ...
    try:
        ...
        await message.answer_document(
            InputFile(preview_path),
            caption="🧪 <b>Test preview</b> (admin) — not shown to end users.",
            parse_mode="HTML",
        )
    except Exception as e:
...
async def process_doc_choice(callback_query: types.CallbackQuery, state: FSMContext):
    """Handle doc_* menu, upsell_doc_* (LTV), retention_upsell_doc_* (24h retention)."""
    ...
    try:
        ...
        # - retention_upsell_doc_* → 24h retention only (RETENTION_CLICK)
        # - upsell_doc_* → LTV cross-sell (POST_PAYMENT_UPSELL_CLICK)
        _raw = callback_query.data
        user_lang = _norm_lang(None, user_id)
        if _raw.startswith(RETENTION_UPSELL_DOC_PREFIX):
            doc_type = _raw[len(RETENTION_UPSELL_DOC_PREFIX):]
            _pending_log = _get_latest_pending(user_id)
            _log_city_doc = None
            if _pending_log and isinstance(_pending_log.get("answers"), dict):
                _log_city_doc = funnel_city_from_mapping(_pending_log.get("answers"))
            log_funnel(
                "RETENTION_CLICK",
                user_id,
                doc_type=doc_type,
        ...
        elif _raw.startswith("upsell_doc_"):
            doc_type = _raw[len("upsell_doc_") :]
            _pending_log = _get_latest_pending(user_id)
            _log_city_doc = None
            if _pending_log and isinstance(_pending_log.get("answers"), dict):
                _log_city_doc = funnel_city_from_mapping(_pending_log.get("answers"))
        ...
        elif _raw.startswith("doc_"):
            doc_type = _raw[len("doc_"):]
        else:
            doc_type = _raw.replace("doc_", "", 1)
...
async def _generate_preview_and_send(
    ...
    logger.info(
        "PREVIEW_GENERATION_STARTED | user=%s doc_type=%s lang=%s use_authority=%s fields=%s",
        user_id, doc_type, user_lang, bool(authority_info),
        len(answers) if answers else 0,
    )
    if not answers:
        logger.warning(
            "PREVIEW_ANSWERS_EMPTY | user=%s doc_type=%s — preview PDF will have no user data",
            user_id, doc_type,
        )
    ...
    _preview_caption = _append_preview_trust_html(_preview_caption, user_lang, uid)
    ...
    _snippet_caption = _append_preview_trust_html(_snippet_caption, user_lang, uid)
...
def register_docs_handlers(dp: "types.Dispatcher"):
    ...
    # doc_* | upsell_doc_* (LTV) | retention_upsell_doc_* (24h retention)
    dp.register_callback_query_handler(
        process_doc_choice,
        lambda c: c.data
        and (
            c.data.startswith("doc_")
            or c.data.startswith("upsell_doc_")
            or c.data.startswith(RETENTION_UPSELL_DOC_PREFIX)
        ),
        state="*",
    )
    dp.register_callback_query_handler(
    ...
    logger.info(
        "✅ Registered handler for doc_*, upsell_doc_*, retention_upsell_doc_*, start_form_*"
    )
    dp.register_callback_query_handler(
