...
async def _extract_city_from_order(order: dict, db=None) -> str:
    """Extract city_code from Stripe session metadata. Falls back to 'berlin'.
...
async def handle_payment_return(message: types.Message, order_id_str: str):
    ...
    """
    logger.info("PAYMENT_RETURN: order_id=%s (silent, awaiting webhook)", order_id_str)
    logger.info(
        "PAYMENT_RETURN: order_id=%s (silent return, webhook handles delivery)",
        order_id_str,
    )
...
async def handle_paid_deeplink(message: types.Message, order_id_str: str):
    ...
    _lang = (get_user_lang(user_id) or "uk").strip().lower() if user_id else "uk"
    ...
    if is_termin_only:
        ...
        try:
            if db.is_order_delivered(order_id):
                ...
                # Webhook already sent the activation UI and marked order as delivered.
                # Do NOT send another message — it would visually bury the monitoring
                # menu that the webhook already rendered with its keyboard.
                # Just unblock the poll loop barrier and return silently.
                logger.info(
                    "TERMIN_IDEMPOTENT_SILENT | user=%s order=%s doc_type=%s",
                    user_id,
                    order_id,
                    doc_type,
                )
    ...
    if is_termin_only:
        ...
        if status in ("sent", "downloaded"):
            ...
            try:
                if _webhook_already_sent:
                    ...
                    _dl_city = _dl_user_row.get("city") or "berlin"
                    _dl_auth = _dl_user_row.get("authority") or "buergeramt"
                    _dl_plan = "7day" if doc_type == "termin_monitor_7day" else "24h"
                    try:
        ...
        if status == "paid":
            ...
            if doc_type in (
                ...
                _paid_city = _paid_user_row.get("city") or "berlin"
                _paid_auth = _paid_user_row.get("authority") or "buergeramt"
                _paid_plan = "7day" if doc_type == "termin_monitor_7day" else "24h"
                try:
            ...
            try:
                ...
                logger.info(
                    "DEEPLINK_ENTITLEMENT_CREATED | user=%s session=%s",
                    _tid,
                    _dl_session,
                )
                # Set reminder_active=1 so _resume_termin_monitoring picks this user on restart
                _dl_user_row = _get_termin_user(_tid) or {}
                _dl_city = _dl_user_row.get("city") or "berlin"
                _dl_auth = _dl_user_row.get("authority") or "buergeramt"
                _crt_reminder(_tid, _dl_city, _dl_auth, 6)
                logger.info(
                    "DEEPLINK_REMINDER_CREATED | user=%s city=%s auth=%s",
                    _tid,
                    _dl_city,
                    _dl_auth,
                )
            ...
            if doc_type == "termin_monitor_24h":
                _sa_city = await _extract_city_from_order(order, db)
                _sa_user_row = _get_termin_user(str(user_id)) or {}
                _sa_auth = _sa_user_row.get("authority") or "buergeramt"
                try:
                    from handlers.termin_activation import send_termin_activation_message as _stam_sa

                    await _stam_sa(
                        message.bot, user_id, _sa_city, _sa_auth, _lang, plan="24h"
                    )
                except Exception as _sa_exc:
                    logger.warning(
                        "TERMIN_24H_ACTIVATION_MSG_FAILED | user=%s err=%s",
                        user_id,
                        _sa_exc,
                    )
                ...
                try:
                    ...
                    if s_status == "complete" and s_pay == "paid":
                        ...
                        try:
                            ...
                            # Set reminder_active=1 so _resume_termin_monitoring picks this user on restart
                            _dl2_user_row = _get_termin_user(_tid) or {}
                            _dl2_city = _dl2_user_row.get("city") or "berlin"
                            _dl2_auth = _dl2_user_row.get("authority") or "buergeramt"
                            _crt_reminder(_tid, _dl2_city, _dl2_auth, 6)
                            logger.info(
                                "DEEPLINK_REMINDER_CREATED | user=%s city=%s auth=%s",
                                _tid,
                                _dl2_city,
                        ...
                        if doc_type in ("termin_monitor_24h", "termin_monitor_7day"):
                            _sp_uid = str(order.get("user_id") or user_id)
                            _sp_city_meta = getattr(session, "metadata", None) or {}
                            _sp_city = (
                                _sp_city_meta.get("city")
                                if hasattr(_sp_city_meta, "get")
                                else None
                            ) or "berlin"
                            _sp_auth_meta = (
                                _sp_city_meta.get("authority")
                                if hasattr(_sp_city_meta, "get")
                                else None
                            )
                            if not _sp_auth_meta:
                                _sp_user_row = _get_termin_user(_sp_uid) or {}
                                _sp_auth_meta = (
                                    _sp_user_row.get("authority") or "buergeramt"
                                )
                            _sp_plan = (
                                "7day" if doc_type == "termin_monitor_7day" else "24h"
                            )
                            try:
                                ...
                                logger.warning(
                                    "TERMIN_STRIPE_PENDING_ACTIVATION_MSG_FAILED | user=%s err=%s",
                                    user_id,
                                    _sp_exc,
                                )
                        elif doc_type == "termin_extend_24h":
