...
_CLOSED_MSG = {
    "uk": "✅ Чат підтримки завершено.",
    "en": "✅ Support chat ended.",
    "de": "✅ Support-Chat beendet.",
    "pl": "✅ Czat wsparcia zakończony.",
    "tr": "✅ Destek sohbeti sona erdi.",
...
async def handle_support_question(message: types.Message, state: FSMContext):
    """Receive user question, call GPT, return answer. State is kept open for follow-up."""
    ...
    kb.row(
        InlineKeyboardButton(_l(_MENU_BTN, lang), callback_data="back_to_main_menu"),
        InlineKeyboardButton(_l(_CLOSE_BTN, lang), callback_data="support_close"),
    )

    await message.answer(response, reply_markup=kb)
    logger.info("SUPPORT_ANSWERED: user_id=%s question_len=%s", message.from_user.id, len(question))
    # State intentionally NOT cleared — user can type the next question directly.
...
# ---------------------------------------------------------------------------
# Mini-chat follow-up handlers
# ---------------------------------------------------------------------------

async def handle_support_close(callback: types.CallbackQuery, state: FSMContext):
    """User tapped '❌ End chat' — clear state, show goodbye, then open main menu."""
    await callback.answer()
    lang = (get_user_lang(callback.from_user.id) or "en").strip().lower()
    if lang == "ua":
        lang = "uk"
    ...
    await state.finish()
    await callback.message.answer(_l(_CLOSED_MSG, lang))
    logger.info("SUPPORT_CLOSED: user_id=%s", callback.from_user.id)
...
def register_support_handlers(dp: Dispatcher):
    # ── open support (generic and contact_support redirect) ─────────────────
    dp.register_callback_query_handler(
        open_ai_support,
        lambda c: c.data in ("ai_support", "contact_support"),
        state="*",
    )
    ...
    dp.register_callback_query_handler(
        ...
        lambda c: c.data == "support_close",
        state=[SupportStates.waiting_for_question, SupportStates.waiting_for_doc_question],
    )

    # ── back to main menu from any support state ────────────────────────────
    dp.register_callback_query_handler(
        handle_support_back,
    ...
    # ── generic support question handler ───────────────────────────────────
